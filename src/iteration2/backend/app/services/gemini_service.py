"""
GeminiService — GDPR-compliant, context-compaction-aware
=========================================================
GDPR Compliance:
...
class GeminiService:
    ...
    async def chat(
        ...
        # GDPR Art. 5(1)(c) + Art. 5(1)(a) — only load EMR if patient has consented
        if emr_consent:
            ...
            clinical_context = None  # No EMR access

        # --- Check if compaction is needed on the current history ---
        was_compacted = False
        new_compacted_summary = compacted_summary  # May be updated below
        ...
        # --- System instruction (GDPR-compliant) ---
        if emr_consent and clinical_context:
            emr_section = (
        ...
        try:
            ...
            return {
                ...
                "emr_fields_used": emr_fields_used,      # GDPR Art. 15
                "was_compacted": was_compacted,
                "new_compacted_summary": new_compacted_summary,  # Pass back for session save
            }
        except Exception as e:
